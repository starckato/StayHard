-- Migration: 008_show_last_login_not_last_log.sql
-- Replace last_log (max daily_logs.log_date) with last_login (auth.users.last_sign_in_at)
-- in the all_users payload. last_log is unreliable because propagateMandatoryDefs()
-- pre-creates future-dated rows when a user adds a mandatory routine — those rows
-- skew "last activity" even though the user didn't actually touch the app.
--
-- last_sign_in_at is updated by Supabase on every session refresh, so it's a true
-- "last time they opened the app" metric.
--
-- Run AFTER 007_cap_future_logs_in_dashboard.sql.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    -- all_users — REPLACED last_log/log_days with last_login (auth.users.last_sign_in_at)
    -- last_login = actual session refresh, reliable proxy for "user opened the app"
    -- Kept log_days but capped at log_date <= current_date for reference.
    'all_users', (
